import * as THREE from 'three';
			...
			VEHICLE_FILEPATHS[0] = 'img/skateboard.glb';
			...
			VEHICLE_FILEPATHS[1] = 'img/ac-alfa-romeo-155-v6-ti/source/Alfa_Romeo_155_V6.fbx';
			...
			const floorTexture = textureLoader.load( "img/asphalt2.jpg" );
			...
			const wallTexture = textureLoader.load( "img/bricks2.jpg" );
...
// "                    "];
			...
			const checkpointMaterial = new THREE.MeshStandardMaterial( {color: 'green', transparent: true, opacity: 0.2} );
			const finishGeometry = new THREE.BoxGeometry( 15, 8, 0.1 );
			const finishMaterial = new THREE.MeshStandardMaterial( {map: textureLoader.load( "img/checkered.jpg" ), transparent: true, opacity: 0.3} );
			const finish = new THREE.Mesh( finishGeometry, finishMaterial );
			const hurdleGeometry = new THREE.BoxGeometry( 100, 0.25, 0.5 );
			const hurdleTexture = textureLoader.load( "img/bricks2.jpg" );
			...
			gltfLoader.load( 'img/OlmecHead.glb', function ( gltf ) {
                ...
                if (elapsed > 16.7) {
                    ...
                    if(vehicleModel) {
                        ...
                        if(keysPressed.left) {
                            ...
                            if(speed > MAX_SPEED[currentVehicle]*.75)
                                speed -= 0.01;
                        ...
                        if(keysPressed.up) {
                            if(speed <= MAX_SPEED[currentVehicle])
                                speed += 0.004;
                        }
                        if(keysPressed.down) {
                            if(speed >= -MAX_SPEED[currentVehicle]/2) 
                        ...
                        if(!finished) {
                            ...
                            ctx.fillText("Time: " + timer.getElapsedTime().toFixed(1) + " s", 20, 120);
                        } else {
                            timer.stop();
                            ctx.clearRect(0, 0, canvas.width, canvas.height);
